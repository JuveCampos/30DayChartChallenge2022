...
curl::curl_download("http://infosiap.siap.gob.mx/gobmx/datosAbiertos/ProduccionAgricola/Cierre_agricola_mun_2020.csv",
                    "d_agg_2020.csv")
...
unique(datos$`Nomcultivo Sin Um`)

# Filtramos para quedarnos con el durazno
peaches_from_mexico = datos %>%
  filter(`Nomcultivo Sin Um` == "Durazno")

# Generamos los datos por estado
edo = peaches_from_mexico %>%
  group_by(Nomestado) %>%
  ...
  ungroup() %>%
  mutate(menos_de_uno = ifelse(pctje < 10, "Otros", Nomestado)) %>%
  group_by(menos_de_uno) %>%
  summarise(pctje_2 = sum(pctje)) %>%
  arrange(-pctje_2) %>%
  ungroup() %>%
  mutate(alpha = 1, edo = menos_de_uno)

edo %>%
  arrange(-pctje_2) %>%
  filter(edo != "Otros") %>%
  pull(edo) %>%
  append("Otros") -> niveles_edo

edo$edo <- factor(edo$edo, niveles_edo)
...
paleta_de_colores <- c(
  ...
  "#BC3231",
  "#752D34")
scales::show_col(paleta_de_colores)
...
plt = edo %>%
  ...
  scale_fill_manual(values = paleta_de_colores) +
  labs(fill = "Estado") +
...
edos_chidos = levels(edo$edo)[-length(edo$edo)]
edos_chidos[2] <-"Michoacán de Ocampo"
